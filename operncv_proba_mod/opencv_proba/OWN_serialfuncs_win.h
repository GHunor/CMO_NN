...
HANDLE CreateHandleSerial(const char* ComName, const char* ID){
...
/// UNUSED
DCB CreateSerialPortParameterProfile(const HANDLE &SerialPort) {
    DCB dcbSerialParams = {0};
...
char* GetByteSeries(HANDLE &hSerial, const int n, DWORD &dwBytesRead){
    char szBuff[
		//n+1       WHY IN THE HELL THIS EXPRESSION DOESNT WORK???
		17
	];
    if(!ReadFile(hSerial, szBuff, n, &dwBytesRead, NULL)){
...
void SetSerialPortParameters(HANDLE &hSerial, int BaudRate = CBR_19200, int ByteSize = 8, int StopBits = ONESTOPBIT, bool Parity = NOPARITY){
...
void SetSerialPortTimeOutProfile(HANDLE &hSerial, COMMTIMEOUTS &timeouts){
    if(!SetCommTimeouts(hSerial, &timeouts)){
        std::cout<<"error occureed with SetCommTimeouts"<<std::endl;
        //error occureed. Inform user
    }
}
void SetByteSeries(HANDLE &hSerial, char* szBuff, const int n, DWORD &dwBytesRead){
...
///16BIT GET AND SET
/*DOESN'T WORK*/void GetByteSeries(HANDLE &hSerial, uint16_t* szBuff, const int n, DWORD &dwBytesRead){  //throws errors
    if(!ReadFile(hSerial, szBuff, n, &dwBytesRead, NULL)){
        std::cout<<"read file error"<<std::endl;
//error occurred. Report to user.
    }
}
/*DOESN'T WORK*/void SetByteSeries(HANDLE &hSerial, uint16_t* szBuff, const int n, DWORD &dwBytesRead){
    if(!WriteFile(hSerial, szBuff, n, &dwBytesRead, NULL)){
        std::cout<<"error occureed 2"<<std::endl;
//error occurred. Report to user.
    }
