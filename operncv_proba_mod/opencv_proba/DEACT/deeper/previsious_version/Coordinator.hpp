...
struct coordinator{
    ...
    std::list<int>::iterator cur, curs, curp;
    bool isPaused, isWait;
	bool WaitEvent, SMEvent;
	...
	coordinator(const char* name){
		...
		if(infile.fail()) std::cout<<"NINCS MOZGAS"<<std::endl;
		bool l = false;
	...
	void statelist_maker(int after_how_many){
        ...
        cur = statelist.begin();
		curs = statelist.begin();
		curp = statelist.begin();
	...
	std::string movement(int state) const {if (state>-1) return movements[state];
        if (state>-(spec_movements.size()+1)) return spec_movements[abs(state)-1];
        return "ERROR";
    }
    ...
    int next_move() const{
		std::list<int>::iterator temp = cur;
        return *(++temp);
    }
	bool isDone() const {
		return cur == statelist.end();}
	...
	void Pause()
    {
        if(isPaused){
			...
			current_goal = wait;
			isWait = true;
			if(curp == statelist.begin()){isWait = false ;update_goal();}
			WaitEvent = true;
			own_iteration_num = 0; 
		}else{//curs=cur;
			cur=statelist.begin();current_goal = -1;}
        isPaused = !isPaused;
    };
    void Repeat_last(){
        cur=curp;
    ...
    void DO( std::string& NEXT) {
        own_iteration_num++;
        if(own_iteration_num == current_goal){
			WaitEvent = true;
			isWait = !isWait;
			if(isWait){
				current_goal = wait;}
        ...
        if(current_goal - own_iteration_num == 2*second) {
            if (isWait) { NEXT = "Prepare for next movement"; SMEvent = true;}
        }else if (WaitEvent){
            NEXT = "";SMEvent = false;
		}else{SMEvent = false;}
    }
