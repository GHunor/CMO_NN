...
SYSTEMTIME syst;
GetSystemTime(&syst);
int systbase = syst.wHour * 3600000 + syst.wMinute * 60000 + syst.wSecond * 1000 + syst.wMilliseconds;
...
public:
	...
	void log(std::ofstream& fajl)
    {
		...
		deltaTime = glove->getLastUpdateTime() - baseT
    }
...
{   
		...
		try {
			...
			while (1) {
				...
				if((GetKeyState('X') & 0x8000/*Check if high-order bit is set (1 << 15)*/) && pressed_X)
				{
					sect=100; //biggest
					break;
				}
